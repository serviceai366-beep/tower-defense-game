...
class UI {
    ...
    applyMultiplayerRoleState() {
        ...
        const menuOpen = this.startScreen && this.startScreen.style.display !== 'none';
    ...
    buildTowerInfoKey(tower) {
        ...
        return [
            ...
            Math.ceil(tower.fortressTimer || 0), Math.ceil(tower.vehicleBuildTimer || 0), tower.vehicleCost, tower.vehicleHp, tower.vehicleSpeed,
            tower.buffPercent, tower.supportBuff,
    ...
    updateTowerButtons() {
        ...
        this.towerButtons.forEach(btn => {
            ...
            const stateKey = [
                unlocked ? 1 : 0,
                unlocked && displayGold < cfg.cost ? 1 : 0,
                hotkey || '',
                cfg.unlockWave || 0,
            ].join('|');
            if (btn.dataset.stateKey === stateKey) return;
    ...
    getTowerDetailText(tower) {
        const detailMap = {
            ...
            factory: 'Система: сборочный цех, бронеплатформа и выездной шлюз к базовой дороге.',
            djBooth: 'Система: двухклеточный микшер, усилители поля и ритм-модуль поддержки.',
    ...
    showTowerInfo(tower) {
        ...
        html += '</div>';
        if (tower.supportBuff > 0) {
            html += `<div class="info-build" style="margin-top:8px;padding:8px;border:1px solid rgba(34,211,238,0.20);border-radius:6px;background:rgba(34,211,238,0.08)">
                <div>🎛 DJ-БАФ АКТИВЕН</div>
                <div style="margin-top:4px;color:#cbd5e1">Все ключевые параметры усилены на <strong>${Math.round(tower.supportBuff * 100)}%</strong>.</div>
            </div>`;
        }
        ...
        this.bindActionPress(document.getElementById('btn-buy-vehicle'), () => {
