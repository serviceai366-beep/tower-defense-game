class Tower {
    ...
    getEffectiveFireRate() {
        const cfg = ABILITY_TYPES.fortress;
        if (!this.isFortified() || !cfg?.fireRateMultiplier) return this.fireRate;
        return this.fireRate / cfg.fireRateMultiplier;
    }
    getBusyProgress() {
    ...
    update(dt, enemies, projectiles) {
        ...
        if (this.isScanner || this.isHealer || this.isFarm || this.isWall) return;
        this.cooldown -= dt;
        if (this.isManualAim) {
            this.target = null;
            return;
    ...
    updatePulseTower(dt, enemies) {
        this.cooldown -= dt;
        const visibleEnemies = enemies.filter(e => !e.isDead && !e.reachedEnd && (!e.isInvisible || this.invisibleDetectionLevel >= (e.invisibilityLevel || 1)) && Math.hypot(e.x - this.x, e.y - this.y) <= this.range);
        ...
        if (window.game) {
            window.game.effects.push(new Effect(this.x, this.y, 'explosion', null, this.range));
            window.game.floatingTexts.push(new FloatingText(this.x, this.y - this.range * 0.25, 'ИМПУЛЬС', '#818cf8'));
        }
    ...
    updateAirfield(dt, enemies, projectiles) {
        ...
        const inZone = enemies.filter(e => {
            if (e.isDead || e.reachedEnd) return false;
            if (e.isInvisible && this.invisibleDetectionLevel < (e.invisibilityLevel || 1)) return false;
            return Math.hypot(e.x - plane.x, e.y - plane.y) <= this.airAttackRadius;
        });
        ...
        if (this.rocketDamage > 0 && this.rocketAoeRadius > 0 && this.rocketTimer <= 0) {
            const rocketTarget = this.pickTarget(enemies.filter(e => {
                if (e.isDead || e.reachedEnd) return false;
                if (e.isInvisible && this.invisibleDetectionLevel < (e.invisibilityLevel || 1)) return false;
                return Math.hypot(e.x - plane.x, e.y - plane.y) <= this.airAttackRadius * 1.35;
            }), plane.x, plane.y);
            if (rocketTarget) {
                projectiles.push(new Projectile(
                    plane.x, plane.y - 12, rocketTarget, this.rocketDamage, '#f59e0b', 8,
                    this, this.rocketAoeRadius, 0, 1, this.rocketSplashBands, 0.54, { style: 'bomb', altitude: 56, lockTargetPoint: true, directHitRadius: 14 }
                ));
                this.rocketTimer = this.rocketCooldownDuration;
    ...
    shoot(projectiles, forcedTarget = null, originX = null, originY = null) {
        ...
        projectiles.push(new Projectile(
            muzzle.x, muzzle.y,
            target, this.damage, cfg.projectileColor, cfg.projectileSize,
            this,
            cfg.isAoe ? this.aoeRadius : 0, this.chainTargets, this.splashDamageMultiplier, this.splashDamageBands, this.projectileSpeedMultiplier
        ));
        if (window.audio) window.audio.playShot(this.type);
    }
    drawRange(ctx) {
    ...
    render(ctx) {
        ...
        if (constructing) {
            ...
            ctx.fillText('+' + this.farmIncome, x, y + 12);
        ...
        if (this.hp < this.maxHp) { const bW2 = bounds.width - 10, bH2 = 3, bx2 = bounds.left + 5, by2 = bounds.top + bounds.height - 6; ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.fillRect(bx2 - 1, by2 - 1, bW2 + 2, bH2 + 2); ctx.fillStyle = hpR > 0.5 ? '#3b82f6' : hpR > 0.25 ? '#f59e0b' : '#ef4444'; ctx.fillRect(bx2, by2, bW2 * hpR, bH2); }
