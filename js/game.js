class SupportVehicle {
    constructor(map, options = {}) {
        SupportVehicle.nextId = SupportVehicle.nextId || 1;
        this.id = options.id || `vh${SupportVehicle.nextId++}`;
        this.ownerId = options.ownerId || 'p1';
        this.level = options.level || 1;
        this.maxHp = options.hp || 160;
        this.hp = options.currentHp ?? this.maxHp;
        this.speed = options.speed || 135;
        this.distance = options.distance ?? map.totalPathLength;
    ...
    updatePosition(map) {
        const pos = map.getPositionAtDistance(Math.max(0, Math.min(map.totalPathLength, this.distance)));
        this.x = pos.x;
        this.y = pos.y;
        this.angle = pos.angle + Math.PI;
    }
    update(dt, game) {
        if (this.isDestroyed || this.reachedEnd) return;
        this.hitFlash = Math.max(0, this.hitFlash - dt * 5);
        this.distance -= this.speed * dt;
        this.updatePosition(game.map);
        const candidates = game.enemies
            .filter(enemy => !enemy.isDead && !enemy.reachedEnd && Math.abs(enemy.distance - this.distance) < 34 && Math.hypot(enemy.x - this.x, enemy.y - this.y) < enemy.radius + 22)
            .sort((a, b) => b.distance - a.distance);
        for (const enemy of candidates) {
            if (this.hp <= 0) break;
            const enemyHp = enemy.hp;
            enemy.takeDamage(this.hp);
            enemy.lastHitOwnerId = this.ownerId;
            this.hp -= Math.min(this.hp, enemyHp);
            this.hitFlash = 0.35;
            game.effects.push(new Effect(enemy.x, enemy.y, 'hit'));
            game.floatingTexts.push(new FloatingText(enemy.x, enemy.y - 24, 'ТАРАН', '#facc15'));
        }
        if (this.hp <= 0) {
            this.hp = 0;
            this.isDestroyed = true;
            game.effects.push(new Effect(this.x, this.y, 'towerDestroyed'));
            game.audio?.playExplosion?.();
        }
        if (this.distance <= 0) this.reachedEnd = true;
    }
    toSnapshot() {
        return {
            id: this.id, ownerId: this.ownerId, level: this.level,
            hp: this.hp, maxHp: this.maxHp, speed: this.speed, distance: this.distance,
            isDestroyed: this.isDestroyed, reachedEnd: this.reachedEnd,
        };
    }
    applySnapshot(data, map, smooth = false) {
        if (!data) return this;
        if (data.id) this.id = data.id;
        this.ownerId = data.ownerId || this.ownerId;
        this.level = data.level || this.level;
        this.hp = data.hp ?? this.hp;
        this.maxHp = data.maxHp ?? data.hp ?? this.maxHp;
        this.speed = data.speed ?? this.speed;
        const nextDistance = data.distance ?? this.distance;
        if (smooth) this.remoteDistance = nextDistance;
        else this.distance = nextDistance;
        this.isDestroyed = !!data.isDestroyed;
        this.reachedEnd = !!data.reachedEnd;
        this.updatePosition(map);
        return this;
    }
    updateRemotePresentation(dt, map) {
        if (this.remoteDistance !== undefined) {
            const gap = this.remoteDistance - this.distance;
            this.distance += gap * Math.min(1, dt * 10);
            if (Math.abs(gap) < 1) this.distance = this.remoteDistance;
        } else {
            this.distance -= this.speed * dt;
        }
        this.hitFlash = Math.max(0, this.hitFlash - dt * 5);
    ...
    render(ctx) {
        if (this.isDestroyed || this.reachedEnd) return;
        const size = this.level >= 3 ? 25 : this.level >= 2 ? 22 : 19;
        ctx.save();
...
class Game {
    ...
    syncVisibleGold(force = false) {
        const visibleGold = this.getDisplayGold();
        if (!force && this.lastVisibleGold === visibleGold) return;
        this.lastVisibleGold = visibleGold;
        this.ui.updateGold(visibleGold);
    }
    ...
    awardPlayerGold(playerKey, amount, options = {}) {
        amount = options.alreadyAdjusted ? amount : this.getCoopMoneyAmount(amount);
        if (!this.isRoomActive()) {
            this.gold += amount;
            this.playerGold.p1 = this.gold;
        } else {
    ...
    buyFactoryVehicle(tower, options = {}) {
        if (!tower || tower.isDestroyed || !tower.isFactory || tower.isBusy() || tower.isDisabled()) return false;
        const ownerId = options.ownerId || options.playerKey || this.getLocalPlayerKey();
        if (!this.canControlTower(tower, ownerId)) return false;
        if (tower.vehicleBuildTimer > 0) return false;
        if (this.isRemoteViewer() && !options.fromNetwork) {
            this.multiplayer?.sendCommand?.({ type: 'buyVehicle', towerId: tower.id, col: tower.col, row: tower.row, towerType: tower.type });
            return true;
        }
        if (!this.spendPlayerGold(ownerId, tower.vehicleCost)) return false;
    ...
    spawnFactoryVehicle(tower) {
        if (!tower || tower.isDestroyed || !tower.isFactory) return;
        const vehicle = new SupportVehicle(this.map, {
            ownerId: tower.vehicleBuildOwnerId || tower.ownerId || 'p1',
            level: tower.level,
            hp: Math.round(tower.vehicleHp * tower.getBuffMultiplier()),
            speed: tower.vehicleSpeed,
        });
        this.vehicles.push(vehicle);
        this.effects.push(new Effect(vehicle.x, vehicle.y, 'hit'));
        this.floatingTexts.push(new FloatingText(vehicle.x, vehicle.y - 22, 'БРОНЕМАШИНА', '#facc15'));
    }
    updateFactoryProduction(dt) {
        for (const tower of this.towers) {
    ...
    updateSupportBuffs() {
        const djBooths = this.towers.filter(t => t.isDjBooth && !t.isDestroyed && !t.isBusy() && !t.isDisabled());
        for (const tower of this.towers) {
            const previousBuff = tower.supportBuff || 0;
            let nextBuff = 0;
            if (!tower.isDestroyed && !tower.isDjBooth) {
                for (const dj of djBooths) {
                    if (Math.hypot(tower.x - dj.x, tower.y - dj.y) <= dj.getEffectiveRange()) {
                        nextBuff = Math.max(nextBuff, dj.buffPercent || 0);
                    }
                }
            }
            if (previousBuff !== nextBuff) {
                const oldMax = Math.max(1, Math.round(tower.maxHp * (1 + previousBuff)));
                const newMax = Math.max(1, Math.round(tower.maxHp * (1 + nextBuff)));
                if (nextBuff > previousBuff && tower.hp >= oldMax - 1) tower.hp = newMax;
                if (tower.hp > newMax) tower.hp = newMax;
                tower.lastSupportBuff = previousBuff;
                tower.supportBuff = nextBuff;
            }
        }
    }
    ...
    getEnemyDifficultyOverrides(type, overrides = {}) {
        ...
        if (this.isCoopActive()) {
            next.hp = Math.max(1, Math.round((next.hp ?? overrides.hp ?? cfg.hp ?? 1) * (CONFIG.COOP_HP_MULTIPLIER || 1.48)));
        }
        return next;
    }
    ...
    resetMatchState(startInMenu = false) {
        this.enemies = []; this.towers = []; this.projectiles = []; this.vehicles = []; this.skyStrikes = []; this.effects = []; this.floatingTexts = [];
        this.towerPlacementSignature = '';
        this.lastVisibleGold = null;
        const baseStartGold = this.getDifficultyConfig().startGold || CONFIG.START_GOLD;
        this.gold = this.isRoomActive() ? Math.round(baseStartGold * (CONFIG.COOP_START_GOLD_MULTIPLIER || 1)) : baseStartGold;
        this.playerGold = { p1: this.gold, p2: this.gold }; this.lives = CONFIG.START_LIVES; this.currentWave = 0; this.maxWaveReached = 0;
    ...
    skipCountdown() {
        ...
        if (bonus > 0) {
            this.awardPlayerGold(this.getLocalPlayerKey(), bonus, { alreadyAdjusted: true });
            const center = this.getViewportCenterWorld();
    ...
    update(dt) {
        ...
        const activeScanners = this.towers.filter(sc => sc.isScanner && !sc.isDestroyed && !sc.isBusy() && !sc.isDisabled());
        for (const t of this.towers) {
            ...
            for (const sc of activeScanners) {
                if (Math.hypot(t.x - sc.x, t.y - sc.y) > sc.scanRadius * sc.getBuffMultiplier()) continue;
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            ...
            if (e.isDead) {
                const killReward = this.getCoopMoneyAmount(e.reward);
                this.awardPlayerGold(e.lastHitOwnerId || this.getLocalPlayerKey(), e.reward);
                this.floatingTexts.push(new FloatingText(e.x, e.y - 15, '+' + killReward, '#d4a017'));
        ...
        this.updateFactoryProduction(dt);
        for (let i = this.vehicles.length - 1; i >= 0; i--) {
            const vehicle = this.vehicles[i];
            vehicle.update(dt, this);
            if (vehicle.isDestroyed || vehicle.reachedEnd) this.vehicles.splice(i, 1);
        }
        const activeHealers = this.towers.filter(h => h.isHealer && !h.isDestroyed && !h.isBusy() && !h.isDisabled());
        for (const t of this.towers) {
            if (t.isHealer || t.isDestroyed || t.hp >= t.getEffectiveMaxHp()) continue;
            let activeHealer = null;
            for (const h of activeHealers) {
                if (Math.hypot(t.x - h.x, t.y - h.y) > h.getEffectiveHealRadius()) continue;
                if (!activeHealer || h.getEffectiveHealRate() > activeHealer.getEffectiveHealRate()) activeHealer = h;
            }
            if (activeHealer) { t.hp = Math.min(t.getEffectiveMaxHp(), t.hp + activeHealer.getEffectiveHealRate() * dt); }
        }
    ...
    updateRemotePresentation(dt) {
        ...
        for (const vehicle of this.vehicles) vehicle.updateRemotePresentation?.(dt, this.map);
        for (let i = this.effects.length - 1; i >= 0; i--) { this.effects[i].update(dt); if (this.effects[i].done) this.effects.splice(i, 1); }
        for (let i = this.floatingTexts.length - 1; i >= 0; i--) { this.floatingTexts[i].update(dt); if (this.floatingTexts[i].done) this.floatingTexts.splice(i, 1); }
    }
    render() {
        ...
        for (const vehicle of this.vehicles) vehicle.render(ctx);
    ...
    drawTowerHoverInfo(ctx, tower) {
        ...
        const supportLabel = tower.isWall ? 'Барьер на дороге' : tower.isHealer ? 'Поддержка: ремонт' : tower.isFarm ? 'Поддержка: ферма' : tower.isFactory ? 'Производство машин' : tower.isDjBooth ? 'Аура усиления' : tower.isScanner ? 'Поддержка: сканер' : tower.isNukeSilo ? 'Ручной пуск' : tower.isManualAim ? 'Ручной выстрел' : 'Поддержка';
        const supportInfo = tower.isWall
            ? `Прочность ${Math.round(tower.hp)} HP`
            : tower.isHealer ? `Хил ${Math.round(tower.getEffectiveHealRate())} HP/с` : tower.isFarm ? `Доход ${tower.getEffectiveFarmIncome()}💰/раунд` : tower.isFactory ? `Машина ${Math.round(tower.vehicleHp * tower.getBuffMultiplier())} HP` : tower.isDjBooth ? `+${Math.round(tower.buffPercent * 100)}% · ${Math.round(tower.getEffectiveRange())}px` : tower.isScanner ? `Вижен ${tower.invisibleDetectionLevel} · ${Math.round(tower.scanRadius * tower.getBuffMultiplier())}px` : tower.isNukeSilo ? `Ракет ${tower.getTotalPayloadCount()} · ${tower.getSelectedPayloadConfig().shortName}` : tower.isManualAim ? `ПКМ · ${tower.fireRate}с` : 'Без урона';
    ...
    applyRemoteCommand(command) {
        ...
        if (command.type === 'buyVehicle') {
            this.buyFactoryVehicle(this.findTowerForNetworkRef(command), {
                ownerId: command.playerKey || command.ownerId || 'p2',
                fromNetwork: true,
            });
            return;
    ...
    createSnapshot() {
        return {
            ...
            vehicles: this.vehicles.filter(v => !v.isDestroyed && !v.reachedEnd).map(v => v.toSnapshot()),
        };
    }
    applySnapshot(snapshot) {
        if (!snapshot || !this.isRemoteViewer()) return;
        const mapChanged = !!(snapshot.mapId && snapshot.mapId !== this.selectedMapId);
        const previousLives = this.lives;
        const previousWave = this.currentWave;
        if (mapChanged) this.selectMap(snapshot.mapId, { fromSnapshot: true });
        ...
        const towersChanged = this.syncTowerSnapshots(snapshot.towers || []);
        this.syncEnemySnapshots(snapshot.enemies || []);
        this.syncVehicleSnapshots(snapshot.vehicles || []);
        if (this.projectiles.length) this.projectiles = [];
        if (mapChanged || towersChanged) this.syncMapOccupancy();
        if (previousLives !== this.lives) this.ui.updateLives(this.lives);
        if (previousWave !== this.currentWave) this.ui.updateWave(this.currentWave);
        this.syncVisibleGold();
        if (towersChanged || previousWave !== this.currentWave) this.ui.updateTowerButtons();
    ...
    syncTowerSnapshots(towerSnapshots) {
        const nextSignature = towerSnapshots
            .map(data => `${data.id || ''}:${data.type}:${data.col}:${data.row}:${data.ownerId || 'p1'}:${data.isDestroyed ? 1 : 0}`)
            .join('|');
        const placementChanged = nextSignature !== this.towerPlacementSignature;
        ...
        this.towerPlacementSignature = nextSignature;
    ...
    syncVehicleSnapshots(vehicleSnapshots) {
        const byId = new Map((this.vehicles || []).map(vehicle => [vehicle.id, vehicle]));
        const next = [];
        for (const data of vehicleSnapshots) {
            let vehicle = data.id ? byId.get(data.id) : null;
            if (!vehicle) {
                vehicle = new SupportVehicle(this.map, {
                    id: data.id,
                ...
                vehicle.applySnapshot(data, this.map, false);
            } else {
                vehicle.applySnapshot(data, this.map, true);
            }
            next.push(vehicle);
        }
        this.vehicles = next;
    ...
    applyCoopHpBoostToAliveEnemies() {
        for (const enemy of this.enemies) {
            ...
            enemy.maxHp = Math.max(1, Math.round(enemy.maxHp * (CONFIG.COOP_HP_MULTIPLIER || 1.48)));
            enemy.hp = Math.max(1, Math.round(enemy.hp * (CONFIG.COOP_HP_MULTIPLIER || 1.48)));
